...
export const handleChangeSubscription = async (payload: Stripe.Subscription) => {
    ...
    const status = (payload.status === "active" || payload.status === "trialing") ? SubscriptionStatus.ACTIVE : payload.status === "canceled" ? SubscriptionStatus.CANCEL : SubscriptionStatus.EXPIRED
